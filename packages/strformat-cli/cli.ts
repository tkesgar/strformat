...
const context = {
  ...
  n2: defineContextFn((_string, _radix?) => {
    const value = Number(_string) || undefined
    return value?.toString(2)
  }),
  n8: defineContextFn((_string, _radix?) => {
    const value = Number(_string) || undefined
    return value?.toString(8)
  }),
  n16: defineContextFn((_string, _radix?) => {
    const value = Number(_string) || undefined
    return value?.toString(16)
  }),
  n36: defineContextFn((_string, _radix?) => {
    const value = Number(_string) || undefined
  ...
  b64u: defineContextFn((_string) => Buffer.from(_string).toString('base64url')),
  ...
  day: defineContextFn((_string, _format?: string) => {
    return dayjs(_string).format(_format?.replaceAll('<', '[').replaceAll('>', ']'))
  }),
  ...(opts.file && (() => {
